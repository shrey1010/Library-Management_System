...
@login_required(login_url='login')
def fine(request):
    # Get all issued items with fines and include author's name
    overdue_items = IssuedItem.objects.filter(
        return_date__lt=date.today(), return_date__isnull=False)
    fines = [{'item': item, 'fine': item.calculate_fine(
    ), 'author_name': item.book_id.author_name} for item in overdue_items]
